...
// ========== EVENT LISTENER DE PAGO - VERSIÓN FINAL ==========
// ========== EVENT LISTENER DE PAGO - VERSIÓN CORREGIDA ==========
/*
function setupPagoListener() {
    const formPago = document.getElementById('formPago');
    if (!formPago) {
        console.log('⏳ Formulario de pago aún no disponible, reintentando en 500ms...');
        setTimeout(setupPagoListener, 500);
        return;
    }
    
    console.log('✅ Configurando event listener de pago');
    
    // Remover listeners anteriores (si existen)
    const newForm = formPago.cloneNode(true);
    formPago.parentNode.replaceChild(newForm, formPago);
    const finalForm = document.getElementById('formPago');
    ...
    finalForm.addEventListener('submit', async function(e) {
        ...
        const monto = parseFloat(this.querySelector('input[name="monto"]')?.value || 0);
        ...
        if (tipoPago === 'nueva' && reservaPendienteStr) {
            const pendiente = JSON.parse(reservaPendienteStr);
            console.log('📦 Creando reserva para:', pendiente);
            ...
            try {
                ...
                if (resultadoReserva?.ok) {
                    const resultadoPago = await procesarPago({
                        ...
                        monto: monto,
                        nombre: this.querySelector('input[name="nombre"]')?.value || 'Cliente',
                        metodo: 'tarjeta'
                    });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Configurar el listener después de que el DOM esté listo
    setTimeout(setupPagoListener, 100);
});
*/
// ========== EVENT LISTENER DEL FORMULARIO DE PAGO - VERSIÓN ÚNICA Y CORRECTA ==========
